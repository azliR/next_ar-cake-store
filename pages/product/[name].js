...
const ItemView = (props) => {
  ...
  const { product } = props
  const { price, image, name, description, qr } = product
  const {
    context: { addToCart },
  } = props
  ...
  return (
    <>
      <CartLink />
      <Head>
        <title>GoBakery ECommerce - {name}</title>
        <meta name="description" content={description} />
        <meta
          property="og:title"
          content={`GoBakery ECommerce - ${name}`}
          key="title"
        />
      </Head>
      <div
      ...
      >
        ...
        <div className="pt-2 px-0 md:px-10 pb-8 w-full md:w-1/2">
          ...
          <Image src={qr} alt="Inventory item" className="max-h-full mb-5 " />
